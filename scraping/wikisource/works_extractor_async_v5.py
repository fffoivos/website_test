...
def is_text_content(url: str) -> bool:
    """Check if the URL points to text content rather than media files or special pages."""
    ...
    # Special wikisource paths to exclude
    special_paths = {
        '/w/', '/wiki/Special:', '/wiki/Category:', '/wiki/Help:',
        '/wiki/Portal:', '/wiki/Template:', '/wiki/Module:',
        '/wiki/MediaWiki:', '/wiki/File:', '/wiki/Talk:',
        '/wiki/User:', '/wiki/User_talk:', '/wiki/Βικιθήκη:',
        '/wiki/Κατηγορία:', '/wiki/Πύλη:', '/wiki/Βοήθεια:',
        '/wiki/Συζήτηση:', '/wiki/Χρήστης:', '/wiki/Πρότυπο:'
    }
    ...
    if any(path in url for path in special_paths):
        return False
        
    # Check for action or oldid parameters
    if '?action=' in url or '&action=' in url or 'oldid=' in url:
        return False
        
    return True

async def process_links(text: str, base_url: str, queue_manager: WorkQueueManager, session: aiohttp.ClientSession, visited_urls: set = None, timeout: int = 300) -> str:
    ...
    try:
                ...
                try:
                    ...
                    if href.startswith('/wiki/') or href.startswith(base_url):
                        try:
                            ...
                            content = await extract_work_content(full_url, queue_manager=queue_manager, session=session, look_for_a_tags=True, is_entry_page=False, visited_urls=visited_urls, timeout=timeout)
                            if content['text']:
                                print("got content ")
                                if line.startswith("• "):
                                    if link.parent:  # Check if link is still in tree
        ...
        # Add final cleanup step to remove any remaining HTML tags
        final_soup = BeautifulSoup(text, 'html.parser')
        text = final_soup.get_text()
...
async def extract_work_content(url: str, queue_manager: WorkQueueManager, session: aiohttp.ClientSession = None, look_for_a_tags: bool = True, is_entry_page: bool = False, visited_urls: set = None, timeout: int = 300) -> Dict[str, Optional[str]]:
    ...
    try:
        ...
        if not content['text'] or content['text'].isspace():
            print(f"No text content found for URL: {url}")
            content['text'] = None
            return content
        ...
        if look_for_a_tags and content['text']:
            content['text'] = await process_links(content['text'], 'https://el.wikisource.org', queue_manager, session, visited_urls, timeout)
    ...
    async with semaphore:
        try:
            ...
            result = {
                ...
                'status': 'success' if content['text'] else 'skipped',
                **row.to_dict()
            }
...
async def process_chunk_async(chunk_df, queue_manager, session, start_idx, semaphore, timeout):
...
async def process_urls_async(input_parquet: str, output_parquet: str, progress_csv: str, 
    ...
    try:
        ...
        async with aiohttp.ClientSession() as session:
            ...
            results = await process_dataset(df, queue_manager, session, progress_csv, 
            ...
            if not new_urls_df.empty:
                ...
                discovered_results = await process_dataset(new_urls_df, discovered_queue_manager,
                                                        session, progress_csv.replace('.csv', '_discovered.csv'),
        ...
        # Initialize empty DataFrames with correct columns if needed
        if results_df.empty:
            results_df = pd.DataFrame(columns=['author', 'author_year', 'title', 'text', 'url'])
        if discovered_df.empty:
            discovered_df = pd.DataFrame(columns=['author', 'author_year', 'title', 'text', 'url'])

        print("\nBefore normalization:")
        print(f"Results unique authors: {results_df['author'].nunique() if not results_df.empty and 'author' in results_df.columns else 0}")
        print(f"Discovered unique authors: {discovered_df['author'].nunique() if not discovered_df.empty and 'author' in discovered_df.columns else 0}")

        # Only process normalization if we have data and required columns
        if not discovered_df.empty and not results_df.empty and 'author' in results_df.columns:
            ...
            if 'author' in discovered_df.columns:
                ...
                if 'author_normalized' in results_df.columns:
                    results_df.drop('author_normalized', axis=1, inplace=True)
                if 'author_normalized' in discovered_df.columns:
        ...
        for col in essential_columns:
            if col not in results_df.columns:
                results_df[col] = None
            if col not in discovered_df.columns:
                discovered_df[col] = None

        # Combine results
        if not discovered_df.empty and set(essential_columns).issubset(discovered_df.columns):
...
async def process_dataset(df: pd.DataFrame, queue_manager: WorkQueueManager, session: aiohttp.ClientSession, 
    ...
    if limit_authors and dataset_type == "original":
        print(f"Before limiting: {len(df)} entries")
        df = df.head(author_num)  # Changed from index-based to head() for clearer limiting
        print(f"After limiting to {author_num} authors: {len(df)} entries")
    else:
    ...
    if processed_indices:
        ...
        print(f"After filtering processed entries: {len(df)} entries to process")
    
    if df.empty:
        print(f"No new entries to process for {dataset_type} dataset")
        return results
    ...
    for chunk_start in range(0, len(df), chunk_size):
        ...
        print(f"Processing {dataset_type} chunk {chunk_start//chunk_size + 1}/{total_chunks}")
        
        chunk_results = await process_chunk_async(chunk_df, queue_manager, session, chunk_start, semaphore, timeout)
