...
def extract_text_from_pdf(file_path):
    ...
    try:
        ...
        logging.error(f"Error extracting text from {file_path}: {type(e).__name__}: {e}")
    return text

def process_pdfs(downloaded_files_path, progress_report_path):
    extracted_files_dir = os.path.join(downloaded_files_path, "pymu_pdf")
    ...
    pdf_files = [f for f in os.listdir(downloaded_files_path) if f.lower().endswith('.pdf')]
    if not pdf_files:
    ...
    for file_name in pdf_files:
        ...
        try:
            ...
            if text:
                ...
                # Ensure the key exists in progress_report, default to "No metadata found" if not
                metadata = progress_report.get(base_name, "No metadata found")
                extracted_files[f"paper_{index}"] = metadata
                index += 1
        except Exception as e:
